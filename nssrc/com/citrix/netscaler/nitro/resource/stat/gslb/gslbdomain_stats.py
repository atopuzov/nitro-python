...
class gslbdomain_stats(base_resource) :
	...
	@property
	def name(self) :
		ur"""Name of the GSLB domain for which to display statistics. If you do not specify a name, statistics are shown for all configured GSLB domains.<br/>Minimum length =  1.
		"""
		try :
			return self._name
	...
	@name.setter
	def name(self, name) :
		ur"""Name of the GSLB domain for which to display statistics. If you do not specify a name, statistics are shown for all configured GSLB domains.
		"""
		try :
			self._name = name
		except Exception as e:
